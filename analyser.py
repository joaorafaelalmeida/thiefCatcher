...
def main():
	...
	print(data[:,1:])
	nSamples,nMetrics=data[:,1:].shape
	print(nMetrics)
	print(extractStatsSilenceActivity(data[:,1:]))
	xxx=data[:,1:]
	silence,activity=extratctSilenceActivity(xxx[:,1],threshold=0)
	print("metrics")
	print(silence)
	print(activity)
	print(len(silence))
	...
	plt.subplot(2,1,1)
	plt.plot(np.array(range(0, len(silence))), np.array(silence))
	plt.subplot(2,1,2)
	plt.plot(np.array(range(0, len(activity))), np.array(activity))
	plt.show()
